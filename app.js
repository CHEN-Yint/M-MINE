...
function loadData() {
  try {
    ...
    if (!raw) { saveData(DEFAULT_DATA); return Object.assign({}, DEFAULT_DATA); }
    const parsed = JSON.parse(raw);
    let migrated = false;
    if (!Array.isArray(parsed.works)) { parsed.works = []; migrated = true; }
    if (typeof parsed.theme !== 'string' || !THEMES[parsed.theme]) { parsed.theme = DEFAULT_THEME; migrated = true; }
...
function saveData(d) {
  ...
  } catch (e) { console.warn('data 写入失败', e); }
}

function applyTheme(themeKey) {
  const root = document.documentElement;
  let safe = THEMES[themeKey] ? themeKey : DEFAULT_THEME;
...
function detectCommentCategory(work) {
  ...
  for (const r of KEYWORD_RULES) {
    if (cats.indexOf(r.cat) !== -1) return r.cat;
...
function renderUser() {
  ...
  atm.style.backgroundImage = user.banner ? `url(${user.banner})` : '';
}
...
function setActiveThemeSwatch(key) {
  const wrap = document.getElementById('theme-swatches');
  if (!wrap) return;
  wrap.querySelectorAll('.theme-swatch').forEach(b => b.classList.toggle('active', b.dataset.theme === key));
}
...
function applyImportedPayload(parsed) {
  ...
  appData = {
    ...
    theme: THEMES[newAppData.theme] ? newAppData.theme : DEFAULT_THEME,
...
function startUploadSimulation(images, caption, categories, ratio) {
  ...
  const timer = setInterval(() => {
    ...
    if (progress >= 100) {
      ...
      setTimeout(() => finalizeUpload(images, caption, categories, ratio), 200);
    }
  }, stepMs);
}

function finalizeUpload(images, caption, categories, ratio) {
  const now = new Date();
  const id = uid();
  const work = {
    id: id, images: images, caption: caption || '',
  ...
  renderStats(); renderGallery(); updateDraftsBadge();
  showToast('发布成功！');
}
...
function renderDots() {
  ...
  if (!carouselImgs.length || carouselImgs.length === 1) { dots.innerHTML = ''; return; }
  dots.innerHTML = carouselImgs.map((_, i) =>
    `<span class="${i === currentSlide ? 'active' : ''}"></span>`
  ).join('');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (bannerInput) bannerInput.addEventListener('change', (e) => {
    ...
    reader.onload = (ev) => {
      ...
      saveUser(user); renderUser(); updateBannerThumb(); showToast('背景已更新');
    };
    reader.readAsDataURL(file);
  });
  ...
  document.getElementById('btn-edit').addEventListener('click', () => {
    ...
    updateBannerThumb();
    setActiveThemeSwatch(appData.theme || DEFAULT_THEME);
  ...
  if (btnImport) btnImport.addEventListener('click', () => openDataIoModal('import', ''));

  const dataIoCancel = document.getElementById('data-io-cancel');
  if (dataIoCancel) dataIoCancel.addEventListener('click', () => closeModal('modal-data-io'));

  const dataIoConfirm = document.getElementById('data-io-confirm');
  if (dataIoConfirm) dataIoConfirm.addEventListener('click', () => {
    const ta = document.getElementById('data-io-text');
    try { applyImportedPayload(JSON.parse(ta.value)); }
    catch (err) { showToast('导入失败：格式不正确'); }
  });
  ...
  document.getElementById('btn-reset-data').addEventListener('click', () => {
    ...
    applyTheme(DEFAULT_THEME); setActiveThemeSwatch(DEFAULT_THEME);
    collectedOnly = false;
  ...
  uploadInput.addEventListener('change', (e) => {
    ...
    files.forEach(file => {
      ...
      const reader = new FileReader();
      reader.onload = (ev) => { pendingImages.push(ev.target.result); renderUploadPreview(); };
      reader.readAsDataURL(file);
    });
    uploadInput.value = '';
  });
  ...
  document.getElementById('upload-confirm').addEventListener('click', () => {
    ...
    resetUpload();
    setTimeout(() => { startUploadSimulation(imagesCopy, caption, categoriesCopy, ratioCopy); }, 50);    
  });
