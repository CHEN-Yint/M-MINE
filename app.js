...
const AI_SETTINGS_KEY = 'mmine_ai_settings_v1';
...
function loadAISettings() {
  try {
    ...
    if (!raw) return Object.assign({}, DEFAULT_AI_SETTINGS);
    const p = JSON.parse(raw);
    return {
      apiBase: typeof p.apiBase === 'string' && p.apiBase ? p.apiBase : DEFAULT_AI_SETTINGS.apiBase,
      apiKey: typeof p.apiKey === 'string' ? p.apiKey : '',
      model: typeof p.model === 'string' && p.model ? p.model : DEFAULT_AI_SETTINGS.model,
      enabled: typeof p.enabled === 'boolean' ? p.enabled : DEFAULT_AI_SETTINGS.enabled
    };
  } catch (e) {
...
function saveAISettings(s) {
  try { localStorage.setItem(AI_SETTINGS_KEY, JSON.stringify(s)); }
  catch (e) { console.warn('AI 设置写入失败', e); }
}
...
function loadUser() {
  try {
    ...
    return {
      ...
      music: (parsed.music && typeof parsed.music === 'object' && parsed.music.url) ? parsed.music : null
    };
...
async function loadData() {
  ...
  if (typeof parsed.theme !== 'string' || (!THEMES[parsed.theme] && !THEMES_PIXEL[parsed.theme] && !THEMES_SPECIAL[parsed.theme])) {
  ...
  parsed.works = parsed.works.map(w => {
    ...
    if (typeof w.location !== 'string') { w.location = ''; migrated = true; }
    ...
    if (!w.aiComments || !w.aiComments.length) {
  w.aiComments = pickCommentsLocal(w); migrated = true;
}
    return w;
  });
...
function applyTheme(themeKey) {
  ...
  const themeSet = isPixel ? THEMES_PIXEL : (isSpecial ? THEMES_SPECIAL : THEMES);
  const fallback = isPixel ? 'pixel-default' : (isSpecial ? 'stardew' : DEFAULT_THEME);
  
  let safe = themeSet[themeKey] ? themeKey : fallback;
...
const DEFAULT_AI_FRIENDS = [
  { id: 'f1', name: '高冷学哥',   avatarEmoji: '🎧', avatarImage: '', style: 'cool',   traits: [] },
  { id: 'f2', name: '温柔邻居',   avatarEmoji: '☕', avatarImage: '', style: 'warm',   traits: [] },
  { id: 'f3', name: '安静的朋友', avatarEmoji: '🌿', avatarImage: '', style: 'quiet',  traits: [] },
  { id: 'f4', name: '话痨学姐',   avatarEmoji: '📚', avatarImage: '', style: 'talk',   traits: [] },
  { id: 'f5', name: '点赞狂魔',   avatarEmoji: '❤️', avatarImage: '', style: 'praise', traits: [] },
  { id: 'f6', name: '文艺诗人',   avatarEmoji: '🎨', avatarImage: '', style: 'art',    traits: [] }
];
...
const KEYWORD_RULES = [
  { keys: ['游戏', '捏人', 'SPECIAL', '角色', '战绩', '抽卡', '出货'], cat: '游戏' },
...
/* ============================================================
   ...
   ------------------------------------------------------------
   每种性格提供多个变体，运行时随机抽一个，
   让同一种性格的朋友在不同作品下说话也不重样。
   ============================================================ */

/* 去掉句尾所有标点 / 空格，方便重新挂尾巴 */
function stripLineTail(line) {
  return String(line == null ? '' : line).replace(/[。！!？?～~、，,：:；;\s]+$/, '');
}

/* 给每一行统一挂尾巴；lastTail 传入时只挂在最后一行 */
function tailLines(lines, allTail, lastTail) {
  const last = lines.length - 1;
  return lines.map(function (l, i) {
    const t = (i === last && lastTail != null) ? lastTail : allTail;
    return stripLineTail(l) + t;
  });
}
...
const PERSONALITY_STYLES = {
  ...
  /* ---------- 高冷：话少、去掉情绪词、收尾干脆 ---------- */
  '高冷': [
    function (lines) { return [removeEmotional(lines[0])]; },
    function (lines) { return [stripLineTail(removeEmotional(lines[0])) + '。']; },
    function (lines) { return [stripLineTail(removeEmotional(lines[0])) + '，还行。']; },
    function (lines) { return [stripLineTail(removeEmotional(lines[0])) + '。嗯。']; },
    function (lines) { return [stripLineTail(removeEmotional(lines[0])) + '，随便看看。']; },
    function (lines) { return [stripLineTail(removeEmotional(lines[0])) + '。知道了。']; }
  ],

  /* ---------- 话痨：感叹号爱好者，越说越兴奋 ---------- */
  '话痨': [
    function (lines) { return tailLines(lines, '！'); },
    function (lines) { return tailLines(lines, '！！'); },
    function (lines) { return tailLines(lines, '呀！'); },
    function (lines) { return tailLines(lines, '哦！'); },
    function (lines) { return tailLines(lines, '！', '～真的！'); },
    function (lines) { return tailLines(lines, '！', '！我跟你说我看了三遍！'); }
  ],

  /* ---------- 温柔：轻声细语，句尾带关心 ---------- */
  '温柔': [
    function (lines) { return tailLines(lines, '。', '，要好好照顾自己哦。'); },
    function (lines) { return tailLines(lines, '。', '，慢慢来就好。'); },
    function (lines) { return tailLines(lines, '。', '，愿你今天也顺顺利利。'); },
    function (lines) { return tailLines(lines, '。', '，记得按时吃饭哦。'); },
    function (lines) { return tailLines(lines, '。', '，有我在呢。'); },
    function (lines) { return tailLines(lines, '。', '，谢谢你愿意分享这些。'); }
  ],

  /* ---------- 毒舌：先反着说，再补一刀 ---------- */
  '毒舌': [
    function (lines) { return [reverseExpress(lines[0])].concat(lines.slice(1)); },
    function (lines) { return [stripLineTail(reverseExpress(lines[0])) + '，就这样吧。']; },
    function (lines) { return [stripLineTail(reverseExpress(lines[0])) + '，别怪我说话直。']; },
    function (lines) { return [stripLineTail(reverseExpress(lines[0])) + '，你自己心里有数。']; },
    function (lines) { return [stripLineTail(reverseExpress(lines[0])) + '。']; }
  ],

  /* ---------- 文艺：句尾挂一句诗 ---------- */
  '文艺': [
    function (lines) { return tailLines(lines, '。', '，' + pickPoetic()); },
    function (lines) { return [pickPoetic()].concat(tailLines(lines, '。')); },
    function (lines) { return tailLines(lines, '。', '，像风一样轻。'); },
    function (lines) { return tailLines(lines, '。', '，' + pickPoetic()); },
    function (lines) { return tailLines(lines, '。', '，都是生活的注脚。'); }
  ],

  /* ---------- Bking：云淡风轻地装一下 ---------- */
  'Bking': [
    function (lines) { return tailLines(lines, '。', '，也就还行吧。'); },
    function (lines) { return tailLines(lines, '。', '，我见得多了。'); },
    function (lines) { return tailLines(lines, '。', '，常规操作。'); },
    function (lines) { return tailLines(lines, '。', '，勉强入眼。'); },
    function (lines) { return tailLines(lines, '。', '，我平时看得比这多。'); }
  ],

  /* ---------- 土纯：实诚、带点口音 ---------- */
  '土纯': [
    function (lines) { return tailLines(lines, '。', '，俺觉得挺好。'); },
    function (lines) { return tailLines(lines, '。', '，俺看着挺得劲儿。'); },
    function (lines) { return tailLines(lines, '。', '，真中！'); },
    function (lines) { return tailLines(lines, '。', '，俺也说不上来，就是喜欢。'); },
    function (lines) { return tailLines(lines, '。', '，这不比啥都强。'); }
  ],

  /* ---------- 安静：字少、句短、留白 ---------- */
  '安静': [
    function (lines) { return [stripLineTail(lines[0]) + '。']; },
    function (lines) { return [stripLineTail(lines[0]) + '……']; },
    function (lines) { return [stripLineTail(lines[0])]; },
    function (lines) { return [stripLineTail(lines[0]) + '。安静地看完了。']; },
    function (lines) { return [stripLineTail(lines[0]) + '。没别的了。']; }
  ],

  /* ---------- 热情：主动、上头、想给你鼓掌 ---------- */
  '热情': [
    function (lines) { return tailLines(lines, '！', '！太棒了！'); },
    function (lines) { return tailLines(lines, '！', '！给你点个赞！'); },
    function (lines) { return tailLines(lines, '！', '！我超喜欢的！'); },
    function (lines) { return tailLines(lines, '！', '！下次带我一个！'); },
    function (lines) { return tailLines(lines, '！'); }
  ],
  ...
  /* ---------- 暖言：稳稳接住你的情绪 ---------- */
  '暖言': [
    function (lines) { return tailLines(lines, '。', '，你已经做得很好了。'); },
    function (lines) { return tailLines(lines, '。', '，别太累着自己。'); },
    function (lines) { return tailLines(lines, '。', '，明天也会是好天气。'); },
    function (lines) { return tailLines(lines, '。', '，慢慢来，不着急。'); },
    function (lines) { return tailLines(lines, '。', '，我一直都在。'); }
  ],
  ...
  '直白': [
    function (lines) { return tailLines(lines, '。'); },
    function (lines) { return [stripLineTail(lines[0]) + '。']; },
    function (lines) { return [stripLineTail(lines[0]) + '，就这样。']; },
    function (lines) { return tailLines(lines.map(removeEmotional), '。'); },
    function (lines) { return [stripLineTail(lines[0]) + '。不吹不黑。']; }
  ]
};

function applyPersonality(lines, trait) {
  if (!trait || !Array.isArray(lines) || !lines.length) return lines;

  const styles = PERSONALITY_STYLES[trait];
  if (!styles || !styles.length) return lines;

  const style = styles[Math.floor(Math.random() * styles.length)];

  let out = null;
  try {
    out = style(lines.slice());
  } catch (e) {
    out = null;
  }

  if (!Array.isArray(out) || !out.length) return lines;

  const cleaned = out.filter(function (l) {
...
/* ---------- 本地评论生成（原 pickCommentsForWork 的逻辑，改名） ---------- */
function pickCommentsLocal(work) {
...
/* ---------- 评论生成核心：优先 DeepSeek（BYOK），失败回退本地 ---------- */
async function pickCommentsForWork(work, useLLM = true) {
  const settings = loadAISettings();
  if (!useLLM || !settings.apiKey) {
    return pickCommentsLocal(work);
  }

  try {
    const friendList = aiFriends.map(f =>
      `${f.name}（性格：${(f.traits || []).join('、') || '普通'}）`
    ).join('、');

    // 读取这次的开关状态（老数据没有这两个字段，默认都读）
const readImage = work.aiReadImage !== false;
const readCaption = work.aiReadCaption !== false;

if (!readImage && !readCaption) {
  // 两个都不读，没必要调 LLM，直接本地
  return pickCommentsLocal(work);
}

let prompt = `你是用户相册里的几位朋友。请以这些朋友的身份${readImage ? '，看这张照片' : ''}${readCaption ? '，结合配文' : ''}，各写一条评论。
朋友列表：${friendList}`;

if (readCaption) {
  prompt += `\n照片配文：${work.caption || '（无）'}`;
  if (work.aiPrompt) prompt += `\n用户提示：${work.aiPrompt}`;
}
...
prompt += `\n分类：${(work.categories || []).join('、') || '（无）'}

要求：
1. 每条评论 15~40 字，口语化、自然，不重复、不说教
2. ${readImage ? '尽量结合照片内容和配文' : '尽量结合配文'}，别泛泛而谈
3. 严格返回 JSON，格式：{"comments":[{"nickname":"朋友名","content":"评论内容"}]}
4. 只返回 JSON，不要任何多余文字`;

// 只有在「读照片」时才把图片发给大模型
const imageParts = readImage
  ...
  : [];
    ...
    const res = await fetch(
      settings.apiBase.replace(/\/$/, '') + '/chat/completions',
      {
        method: 'POST',
        headers: {
    ...
    if (!Array.isArray(parsed.comments) || !parsed.comments.length) {
      throw new Error('返回评论为空');
    }

    const friendMap = {};
    aiFriends.forEach(f => { friendMap[f.name] = f; });

    return parsed.comments.map(c => {
      const f = friendMap[c.nickname];
      return {
        avatar: f ? (f.avatarImage || f.avatarEmoji) : '🤖',
        avatarIsImage: f ? !!f.avatarImage : false,
        nickname: c.nickname || 'AI 朋友',
        content: c.content || '',
        time: '刚刚',
        commentId: 'c-' + uid(),
        friendId: f ? f.id : ''
    ...
    console.warn('AI 调用失败，回退本地', e);
    showToast('AI 调用失败，已用本地评论代替');
    return pickCommentsLocal(work);
  }
}

async function generateAIComments(work) { return await pickCommentsForWork(work); }
...
function renderUser() {
  ...
  document.getElementById('bio').textContent = user.bio || '今天是幸福日!';
  applyBanner();
  renderMusicPlayer();
} 
...
function renderThemeSwatches() {
  ...
  const isSpecial = style === 'special';
  const themeSet = isPixel ? THEMES_PIXEL : (isSpecial ? THEMES_SPECIAL : THEMES);
  const currentTheme = appData.theme || (isPixel ? 'pixel-default' : (isSpecial ? 'stardew' : 'default'));
...
function applyImportedPayload(parsed) {
  ...
  user = {
    ...
    theme: (THEMES[newAppData.theme] || THEMES_PIXEL[newAppData.theme] || THEMES_SPECIAL[newAppData.theme]) ? newAppData.theme :  DEFAULT_THEME,
...
function renderAIFriendsList() {
  ...
  style: aiFriends[idx].style || '', traits: newTraits
};
        const cleanList = aiFriends.map(x => ({
        id: x.id, name: x.name, avatarEmoji: x.avatarEmoji, avatarImage: x.avatarImage, style: x.style,
        traits: Array.isArray(x.traits) ? x.traits : [],
...
const newFriendState = { emoji: '😊', image: '', traits: [] };
...
function initAddFriendPanel() {
 ...
 const newFriend = {
  ...
  style: '', traits: traits
      ...
      saveAIFriends(cleanList); appData.aiFriends = cleanList; saveData(appData);
      newFriendState.emoji = '😊'; newFriendState.image = ''; newFriendState.traits = [];
...
function parseMusicLink(input) {
  ...
  /* ---------- 网易云 · 单曲 ---------- */
  let m = url.match(/music\.163\.com.*[?&]id=(\d+)/i);
  if (m && !/playlist/i.test(url)) {
    return {
      platform: 'netease',
      type: 'song',
      id: m[1],
      url: url,
      embed: 'https://music.163.com/outchain/player?type=2&id=' + m[1] + '&auto=0&height=66',
      height: 86
    };
  }

    /* ---------- 网易云 · 歌单 ---------- */
  m = url.match(/music\.163\.com.*playlist.*[?&]id=(\d+)/i);
  if (m) {
    return {
      platform: 'netease',
      type: 'playlist',
      id: m[1],
      url: url,
      embed: 'https://music.163.com/outchain/player?type=0&id=' + m[1] + '&auto=0&height=240', // 把 430 改成 240
      height: 240 // 把 430 改成 240
    };
  }

  /* ---------- Spotify · 单曲 ---------- */
  m = url.match(/open\.spotify\.com\/(?:intl-[a-z-]+\/)?track\/([a-zA-Z0-9]+)/i);
  if (m) {
    return {
      platform: 'spotify',
      type: 'track',
      id: m[1],
      url: url,
      embed: 'https://open.spotify.com/embed/track/' + m[1],
      height: 152
    };
  }

  /* ---------- Spotify · 歌单 / 专辑 ---------- */
  m = url.match(/open\.spotify\.com\/(?:intl-[a-z-]+\/)?(playlist|album)\/([a-zA-Z0-9]+)/i);
  if (m) {
    return {
      platform: 'spotify',
      type: m[1],
...
function renderMusicPlayer() {
  const wrap = document.getElementById('music-player');
  if (!wrap) return;
  ...
  const nameEl = document.getElementById('music-player-name');
  nameEl.textContent = MUSIC_PLATFORM_LABEL[m.platform] || '音乐';

  const bodyEl = document.getElementById('music-player-body');
  bodyEl.innerHTML = '';

  const iframe = document.createElement('iframe');
  iframe.src = m.embed;
  iframe.style.height = (m.height || 86) + 'px';
  iframe.setAttribute('frameborder', '0');
  iframe.setAttribute('allow', 'autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture');
  iframe.setAttribute('loading', 'lazy');
  bodyEl.appendChild(iframe);

  const header = wrap.querySelector('.music-player-header');
...
async function generateShareLink(options = {}) {
  ...
  const res = await fetch('/api/share', {
    ...
    body: JSON.stringify({ data: payload, burn: burn, ttlHours: 24 })
  });

  if (!res.ok) throw new Error('生成失败');
  const { id } = await res.json();
  const baseUrl = `${location.origin}${location.pathname}`;
  return `${baseUrl}#/s/${id}${burn ? '?burn=1' : ''}`;
}
let pendingImages = [];
...
let pendingAiPrompt = '';
let pendingAiReadImage = true;
let pendingAiReadCaption = true;
...
// ★ 「朋友评论」总开关联动：关掉时，下面两个子开关一起关且禁用
function syncAiSubToggles() {
  const aiToggle = document.getElementById('upload-ai-comment-toggle');
  const imgToggle = document.getElementById('upload-ai-read-image-toggle');
  const capToggle = document.getElementById('upload-ai-read-caption-toggle');
  const imgRow = document.getElementById('upload-ai-read-image-row');
  const capRow = document.getElementById('upload-ai-read-caption-row');
  if (!aiToggle || !imgToggle || !capToggle) return;

    const on = aiToggle.checked;
  imgToggle.checked = on;
  capToggle.checked = on;
  imgToggle.disabled = !on;
  capToggle.disabled = !on;
  if (imgRow) imgRow.style.opacity = on ? '1' : '0.4';
  if (capRow) capRow.style.opacity = on ? '1' : '0.4';
}
...
function resetUpload() {
  ...
  pendingAiReadImage = true;
  pendingAiReadCaption = true;
  const imgToggle = document.getElementById('upload-ai-read-image-toggle');
  if (imgToggle) imgToggle.checked = true;
  const capToggle = document.getElementById('upload-ai-read-caption-toggle');
  if (capToggle) capToggle.checked = true;
  syncAiSubToggles();
  const aiPromptInput = document.getElementById('upload-ai-prompt');
  if (aiPromptInput) aiPromptInput.value = '';
  const locInput = document.getElementById('upload-location');
...
function startUploadSimulation(images, caption, categories, ratio, aiPrompt, aiEnabled, location, readImage, readCaption) {
  ...
  const timer = setInterval(() => {
    ...
    if (progress >= 100) {
      ...
      setTimeout(async () => {
  try {
    await finalizeUpload(images, caption, categories, ratio, aiPrompt, aiEnabled, location, readImage, readCaption);
  } catch (e) {
    console.warn('finalizeUpload 失败', e);
...
async function finalizeUpload(images, caption, categories, ratio, aiPrompt, aiEnabled, location, readImage, readCaption) {
  const now = new Date();
  const id = uid();
  const work = {
  id: id,
  ...
  aiReadImage: readImage !== false,      // 默认 true
  aiReadCaption: readCaption !== false   // 默认 true
};

  // 先插入占位评论
 const settings = loadAISettings();
const willUseLLM = aiEnabled && settings.enabled && !!settings.apiKey;   // ← 加 settings.enabled
...
if (willUseLLM) {
  ...
  work.aiComments = pickCommentsLocal(work);
} else {
  work.aiComments = [];
}
  ...
  if (willUseLLM) {
  showToast('正在生成评论...');
  const comments = await pickCommentsForWork(work);
  work.aiComments = comments;
  saveData(appData);
  if (currentDetailId === work.id) {
...
function openDetail(id) {
  ...
  if (dm) dm.classList.remove('show-comments');
  ...
  if (locEl) {
    if (work.location) {
      locEl.textContent = '📍 ' + work.location;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bioCounter = document.getElementById('bio-counter');
   ...
   document.getElementById('edit-ai-api-base').value = s.apiBase;
   document.getElementById('edit-ai-api-key').value = s.apiKey;
   document.getElementById('edit-ai-model').value = s.model;
   document.getElementById('edit-ai-enabled-toggle').checked = s.enabled;
    bioCounter.textContent = `${(user.bio || '').length} / 60`;
    updateBannerThumb();
    document.getElementById('edit-music-url').value = (user.music && user.music.url) || '';
    renderThemeSwatches();
  ...
  document.getElementById('edit-save').addEventListener('click', () => {
    ...
    if (!newName) { showToast('用户名不能为空'); return; }
        const musicUrl = document.getElementById('edit-music-url').value.trim();
    if (musicUrl) {
      const parsed = parseMusicLink(musicUrl);
      if (!parsed) {
        showToast('无法识别，请检查是否为网易云或 Spotify 链接');
        return;
      }
      user.music = parsed;
    } else {
    ...
    user.bio = newBio || '今天是幸福日！';
   saveAISettings({
  apiBase: (document.getElementById('edit-ai-api-base').value || '').trim() || DEFAULT_AI_SETTINGS.apiBase,
  apiKey: (document.getElementById('edit-ai-api-key').value || '').trim(),
  model: (document.getElementById('edit-ai-model').value || '').trim() || DEFAULT_AI_SETTINGS.model,
  enabled: document.getElementById('edit-ai-enabled-toggle').checked 
});
  ...
  if (btnImport) {
    btnImport.addEventListener('click', () => {
      openDataIoModal('import', '');
    });
  }
  
    // 绑定导入确认按钮
  const btnIoConfirm = document.getElementById('data-io-confirm');
  if (btnIoConfirm) {
    btnIoConfirm.addEventListener('click', () => {
      const text = document.getElementById('data-io-text').value;
      if (!text.trim()) { showToast('请先粘贴备份数据'); return; }
      try {
        const parsed = JSON.parse(text);
        applyImportedPayload(parsed);
        closeModal('modal-data-io');
      } catch (e) {
...
document.getElementById('ai-close').addEventListener('click', () => closeModal('modal-ai'));
 document.getElementById('btn-share').addEventListener('click', async () => {
  try {
    ...
    if (appData.works.length > 3) {
      if (!confirm('你的作品较多（超过 3 张），生成分享链接会占用较多存储空间，确定继续吗？')) {
        return;
      }
    }

    const url = await generateShareLink({ burn });
    document.getElementById('share-link').textContent = url;
    openModal('modal-share');
  } catch (e) {
    console.error(e);
    showToast('生成失败，请重试或检查网络');
  }
});
...
// 总开关 change 时联动（syncAiSubToggles 在外部定义）
document.getElementById('upload-ai-comment-toggle').addEventListener('change', syncAiSubToggles);
// 首次进页面同步一次
syncAiSubToggles();
  ...
  document.getElementById('upload-cancel').addEventListener('click', () => { closeModal('modal-upload'); resetUpload(); });
    ...
    const readImage = document.getElementById('upload-ai-read-image-toggle').checked;
    const readCaption = document.getElementById('upload-ai-read-caption-toggle').checked;
    ...
    resetUpload();
    setTimeout(() => { startUploadSimulation(imagesCopy, caption, categoriesCopy, ratioCopy, aiPrompt, aiEnabled, location, readImage, readCaption); }, 50);       
  });

  // ★ 定位按钮（独立绑定，与"发布"无关）
  (function bindLocationButton() {
    const btn = document.getElementById('upload-location-btn');
    const input = document.getElementById('upload-location');
    if (!btn || !input) return;
  ...
  document.getElementById('act-collect').addEventListener('click', () => {
    ...
    if (!modal) return;
    const isOpen = modal.classList.contains('show-comments');
    if (isOpen) {
      // 收起评论 → 回到单列布局
      modal.classList.remove('show-comments');
    } else {
      // 展开评论 → 切成左右分栏
      modal.classList.add('show-comments');
...
// 风格切换器初始化
let savedStyle = localStorage.getItem('mmine_style') || 'default';
if (savedStyle === 'sims' || savedStyle === 'SPECIAL') savedStyle = 'special';  // 兼容旧命名
document.documentElement.dataset.style = savedStyle;const styleBtns = document.querySelectorAll('.style-btn');
styleBtns.forEach(btn => {
  ...
  btn.addEventListener('click', () => {
    ...
    const isSpecial = styleName === 'special';
    const validThemes = isPixel ? THEMES_PIXEL : (isSpecial ? THEMES_SPECIAL : THEMES);
    const fallback    = isPixel ? 'pixel-default' : (isSpecial ? 'stardew' : 'default');

    // 智能切换主题（跨风格时自动落到目标风格默认主题）
    let targetTheme = appData.theme;
    if (!validThemes[targetTheme]) targetTheme = fallback;
  ...
  (async function loadAndRender() {
    // 1. 检测是否是分享链接（格式： #/s/xxxxxx?burn=1）
    const hashMatch = location.hash.match(/^#\/s\/([a-zA-Z0-9]+)(\?burn=1)?$/);
    
    if (hashMatch) {
      const id = hashMatch[1];
      const burn = hashMatch[2] ? '1' : '0';
      
      try {
        const res = await fetch(`/api/share?id=${id}&burn=${burn}`);
        
        if (!res.ok) {
          const err = await res.json();
          document.body.innerHTML = `
            <div style="padding:80px 24px;text-align:center;font-family:sans-serif;">
              <h2 style="font-size:18px;margin-bottom:8px;">${err.error || '链接无效'}</h2>
              <p style="color:#888;font-size:13px;">请向分享者索要新的链接</p>
            </div>`;
          return;
        }
        
        const { data } = await res.json();
        appData.works = data.w || [];
        user = Object.assign({}, user, data.u || {});
        if (data.s) {
          document.documentElement.dataset.style = data.s;
        }
        if (data.t) applyTheme(data.t);
        
        // 隐藏编辑和上传按钮，进入纯浏览模式
        document.getElementById('btn-edit').style.display = 'none';
        document.getElementById('fab-add').style.display = 'none';
        document.getElementById('fab-drafts').style.display = 'none';
        
        renderAll();
        renderThemeSwatches();
        showToast('正在查看分享内容（只读）');
        return;
      } catch (e) {
...
/* ★ 逆地理编码：经纬度 → 地名（用 OpenStreetMap Nominatim，免费无需 key） */
async function reverseGeocode(lat, lon) {
  try {
    const url = `https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}&accept-language=zh-CN&zoom=12`;
    ...
    console.warn('reverseGeocode 失败', e);
    return '';
  }
}