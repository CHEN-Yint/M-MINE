...
function startUploadSimulation(images, caption, categories, ratio, aiPrompt, aiEnabled, location) {
  ...
  const timer = setInterval(() => {
    ...
    if (progress >= 100) {
      ...
      setTimeout(() => finalizeUpload(images, caption, categories, ratio, aiPrompt, aiEnabled, location), 200);
    }
  }, stepMs);
}

function finalizeUpload(images, caption, categories, ratio, aiPrompt, aiEnabled, location) {
  const now = new Date();
  const id = uid();
  const work = {
    id: id, 
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('upload-cancel').addEventListener('click', () => { closeModal('modal-upload'); resetUpload(); });
    ...
    resetUpload();
    setTimeout(() => { startUploadSimulation(imagesCopy, caption, categoriesCopy, ratioCopy, aiPrompt, aiEnabled, location); }, 50);        
  });
